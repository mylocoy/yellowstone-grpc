...
impl GeyserPlugin for Plugin {
    ...
    fn on_load(&mut self, config_file: &str, is_reload: bool) -> PluginResult<()> {
        ...
        let (parallel_encoder, encoder_handle) = ParallelEncoder::new(encoder_threads);
        log::info!("parallel encoder enabled with {encoder_threads} threads");
        ...
        let (snapshot_channel, grpc_channel) = match result {
            Ok(value) => value,
            Err(error) => {
                log::error!("failed to start plugin services: {error}");
    ...
    fn on_unload(&mut self) {
        if let Some(inner) = self.inner.take() {
            ...
            if let Err(error) = inner.encoder_handle.join() {
                log::error!("encoder thread panicked: {:?}", error);
            }
